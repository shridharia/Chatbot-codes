...
var API_URL = window.location.hostname === "localhost" ? "https://impact-agents.ai" : "https://impact-agents.ai";
var type_mode = false

async function loadChatbotContent() {
    try {
        const response = await fetch('http://localhost:8000' + '/ia_chatbot_embed', {
            method: 'GET',
            headers: {
	...
	async resetSessionMemory() {
        try {
           ...
           addMessage(content, type) {
               ...
               let ampm = '';
               // Use 12-hour format if desired, otherwise comment the next 2 lines
               // ampm = hours >= 12 ? 'PM' : 'AM';
               // hours = hours % 12 || 12;
               ...
               if (type === 'bot' && content.includes('Sources:')) {
                   ...
                   messageContent.innerHTML = (type === 'bot') ? this.formatLinks(content) : content;
               }

               // For bot messages, render meta row outside the message bubble
               if (type === 'bot') {
